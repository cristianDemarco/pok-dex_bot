...
class PokemonAPI:
    ...
    def get_api_data(self, pokemon : str = "Undefined", variety : int = 0) -> None:
        ...
        if variety > species_data_response["varieties"].json().length:
            variety = 0
            
        first_pokemon_variety = species_data_response["varieties"][variety]["pokemon"]["name"]
        data_response = requests.get(f"https://pokeapi.co/api/v2/pokemon-species/{first_pokemon_variety}")
