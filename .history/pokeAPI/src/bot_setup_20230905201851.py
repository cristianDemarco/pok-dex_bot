...
async def search_pokemon(update: Update, context: ContextTypes.DEFAULT_TYPE, is_Callback : bool = False) -> None:
    update.callback_query.data = json.loads(update.callback_query.query.data)
    ...
    keyboard = [
        [
            InlineKeyboardButton(text = f"< N°{int(pokemon.id) - 1}", callback_data=str(
                    {
                        "pokemon" : f"/pokemon {int(pokemon.id) - 1}"
                    }
                )
            ),
            InlineKeyboardButton(text = f"Change Variety", callback_data=str(            
                    {
